...
COPD=as.data.frame(read.table("COPD_diff_all_1400_metacore_FDR0.05.txt",header=T,sep="\t"))

Silicosis$"Pathway"=as.character(Silicosis$"Pathway")
COPD$"Pathway"=as.character(COPD$"Pathway")

merge <- list(Silicosis=Silicosis, COPD=COPD)
...
pdf("catLow2HighNoRefByEqualRanks03Prop.pathway.sili-COPD.pdf",width=10)
